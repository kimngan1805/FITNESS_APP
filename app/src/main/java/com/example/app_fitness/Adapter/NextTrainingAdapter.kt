...
class NextTrainingAdapter(
    ...
    private var completedExercises = mutableSetOf<Int>() // Lưu trữ ID các bài tập đã hoàn thành

    fun updateCompletedExercises(completed: Set<Int>) {
        completedExercises = completed.toMutableSet()
    ...
    inner class NextTrainingViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            itemView.setOnClickListener {
                ...
                if (isExerciseUnlocked(exercise.unlock_order)) {
                    onExerciseClicked(exercise)
                } else {
                    Toast.makeText(context, "Hoàn thành bài tập trước đó!", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }

    private fun isExerciseUnlocked(unlockOrder: Int): Boolean {
        return unlockOrder == 1 || completedExercises.contains(unlockOrder - 1)
    }
    ...
    override fun onBindViewHolder(holder: NextTrainingViewHolder, position: Int) {
        ...
        if (isExerciseUnlocked(exercise.unlock_order)) {
            ...
            holder.statusIcon.setColorFilter(null) // Loại bỏ tint màu xám
        } else {
            holder.statusIcon.setImageResource(R.drawable.ic_lock)
            holder.statusIcon.setColorFilter(context.resources.getColor(R.color.blue), android.graphics.PorterDuff.Mode.SRC_IN) // Tint màu xám
        }
    }
